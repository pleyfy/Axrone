...
export interface WorldDiagnosticsSnapshot {
    readonly entityCount: number;
    readonly archetypeCount: number;
...
export interface WorldDebugInfoSnapshot extends WorldDiagnosticsSnapshot {
...
export class WorldDiagnostics {
    ...
    getMetrics(snapshot: WorldDiagnosticsSnapshot): Readonly<WorldMetrics> | null {
        if (!this._enabled) {
            return null;
        }
    ...
    private _estimateMemoryUsage(snapshot: WorldDiagnosticsSnapshot): number {
        let totalSize = 0;
