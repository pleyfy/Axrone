import { IVec2Like } from '@axrone/numeric';
import { AABB2D, AABB } from '../aabb';
import { ISpatialPartitioning2D, ISpatialNode } from './interfaces';
import {
    ISpatialQueryResult,
    IRaycastHit,
    ISpatialConfig,
...
interface IQuadTreeEntry<TItem> {
    readonly item: TItem;
    readonly bounds: AABB2D;
    readonly id: symbol;
}

export class QuadTree<TItem> implements ISpatialPartitioning2D<TItem> {
    private readonly _root: QuadTreeNode<TItem>;
    private readonly _config: Readonly<ISpatialConfig>;
    private readonly _itemMap = new Map<TItem, IQuadTreeEntry<TItem>>();
    private _nodeCount = 1;
    private _version = 0;

    constructor(bounds: readonly [IVec2Like, IVec2Like], config?: Partial<ISpatialConfig>) {
        this._config = Object.freeze({ ...DEFAULT_SPATIAL_CONFIG, ...config });
        this._validateConfig();
        this._validateBounds(bounds);

        const aabb = AABB.create2D(bounds[0], bounds[1]);
        this._root = new QuadTreeNode<TItem>(aabb, 0, this._config, () => ++this._nodeCount);
    }
    ...
    public insert(bounds: readonly [IVec2Like, IVec2Like], item: TItem): void {
        if (this._itemMap.has(item)) {
            throw new SpatialItemError('Item already exists in QuadTree', {
                item,
                size: this.size,
            });
        }
        ...
        const aabb = AABB.create2D(bounds[0], bounds[1]);
        const entry: IQuadTreeEntry<TItem> = {
            item,
            bounds: aabb,
            id: Symbol('qt-item'),
        };
    ...
    public query(bounds: readonly [IVec2Like, IVec2Like]): ISpatialQueryResult<TItem>[] {
        this._validateBounds(bounds);

        const queryBounds = AABB.create2D(bounds[0], bounds[1]);
    ...
    private _validateBounds(bounds: readonly [IVec2Like, IVec2Like]): void {
        if (bounds.length !== 2) {
    ...
    private _estimateMemoryUsage(): number {
        return this._nodeCount * 200 + this._itemMap.size * 100;
    }
}

class QuadTreeNode<TItem> {
    private _children:
        | readonly [
              QuadTreeNode<TItem>,
              QuadTreeNode<TItem>,
    ...
    private readonly _entries = new Set<IQuadTreeEntry<TItem>>();

    constructor(
        public readonly bounds: AABB2D,
    ...
    public insert(entry: IQuadTreeEntry<TItem>): boolean {
    ...
    public remove(entry: IQuadTreeEntry<TItem>): boolean {
    ...
    public query(queryBounds: AABB2D, results: ISpatialQueryResult<TItem>[]): void {
    ...
    private _canSubdivide(): boolean {
        ...
        return extents.x >= this._config.minNodeSize && extents.y >= this._config.minNodeSize;
    }
    ...
    private _subdivide(): void {
        ...
        const extents = this.bounds.extents;
        const childExtents = { x: extents.x * 0.5, y: extents.y * 0.5 };

        const childConfigs: readonly [IVec2Like, IVec2Like][] = [
            [{ x: center.x - childExtents.x, y: center.y - childExtents.y }, childExtents],
            [{ x: center.x + childExtents.x, y: center.y - childExtents.y }, childExtents],
            [{ x: center.x - childExtents.x, y: center.y + childExtents.y }, childExtents],
            [{ x: center.x + childExtents.x, y: center.y + childExtents.y }, childExtents],
        ] as const;
        ...
        this._children = childConfigs.map(([childCenter, childExtents]) => {
            this._onNodeCreate();
            return new QuadTreeNode<TItem>(
                AABB.fromCenterAndExtents2D(childCenter, childExtents),
                this.depth + 1,
                this._config,
                this._onNodeCreate
            );
...
export namespace QuadTree {
    export function create<TItem>(bounds: readonly [IVec2Like, IVec2Like]): QuadTree<TItem> {
        return new QuadTree<TItem>(bounds);
    }

    export function createForSmallObjects<TItem>(
        bounds: readonly [IVec2Like, IVec2Like]
    ): QuadTree<TItem> {
        return new QuadTree<TItem>(bounds, {
            maxDepth: 8,
            maxItemsPerNode: 32,
