...
const uiInputFocusExample: SceneExample = {
    ...
    async mount({ container }: ExampleContext) {
        ...
        const panel = createDemoPanel(runtime, { width: 548, height: 328, gap: 14 });
        const title = createDemoText(runtime, 'INPUT AND FOCUS', 20, { color: '#f8fafcff', layout: { height: 26 } });
        const instructions = createDemoText(runtime, 'TAB ARROWS CLICK TYPE BACKSPACE', 12, { color: '#93c5fdff', layout: { height: 16 } });
        const actionRow = runtime.createWidget({ layout: { display: 'stack', direction: 'row', gap: 12, height: 58 } });
        const inputCard = runtime.createWidget({
            layout: { padding: 14, display: 'stack', direction: 'column', gap: 10, height: 88 },
            style: { background: '#111827ff', borderColor: '#334155ff', borderWidth: 1, radius: 14 },
        });
        const statusCard = runtime.createWidget({
            layout: { padding: 14, display: 'stack', direction: 'column', gap: 8, height: 94 },
            style: { background: '#111827ff', borderColor: '#334155ff', borderWidth: 1, radius: 14 },
        });
        ...
        const inputWidget = runtime.createWidget({
            ...
            layout: { height: 42, padding: [12, 12] },
            ...
            handlers: {
                ...
                keyDown: (event) => {
                    if (event.key === 'Backspace' && state.cursor > 0) {
                    ...
                    if (event.key === 'Delete' && state.cursor < state.value.length) {
        ...
        const selectedLabel = createDemoText(runtime, `ACTION ${state.selectedAction}`, 14, { color: '#7dd3fcff', layout: { height: 18 } });
        const logLabel = createDemoText(runtime, state.log, 12, { color: '#cbd5e1ff', layout: { height: 34 }, text: { wrap: 'word' } });
        ...
        const createActionButton = (action: string) => {
            ...
            widget = runtime.createWidget({
                ...
                layout: { grow: 1, height: 42, padding: [10, 12] },
            ...
            label = createDemoText(runtime, action, 14, { color: '#93c5fdff', layout: { height: 18 } });
        ...
        runtime.appendChild(panel, inputCard);
        runtime.appendChild(panel, statusCard);
        ...
        runtime.appendChild(inputCard, createDemoText(runtime, 'TEXT FIELD', 14, { color: '#f8fafcff', layout: { height: 18 } }));
        runtime.appendChild(inputCard, inputWidget);

        runtime.appendChild(statusCard, createDemoText(runtime, 'STATUS', 14, { color: '#f8fafcff', layout: { height: 18 } }));
