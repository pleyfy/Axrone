...
import type { BodyType, IMaterial, SolverFlags, IRaycastResult3D } from '../types';
...
export class BodyManager3D implements Disposable {
    ...
    private readonly _bodyIdToIndex: Map<BodyId3D, number>;
    private readonly _indexToBodyId: Map<number, BodyId3D>;
...
export class ShapeManager3D implements Disposable {
    ...
    private readonly _shapeIdToIndex: Map<ShapeId3D, number>;
    private readonly _shapeToBody: Map<ShapeId3D, BodyId3D>;
    private readonly _bodyToShapes: Map<BodyId3D, Set<ShapeId3D>>;
    ...
    constructor(maxShapes: number = 8192) {
        ...
        this._shapeIdToIndex = new Map();
        this._shapeToBody = new Map();
        ...
        this._aabbs = new Float32Array(maxShapes * AABB_STRIDE);
    }
...
export class ConstraintManager3D implements Disposable {
    ...
    private readonly _constraintIdToIndex: Map<ConstraintId3D, number>;
    private readonly _constraintTypes: Uint8Array;
    private readonly _constraintData: Float64Array;
    private readonly _bodyToConstraints: Map<BodyId3D, Set<ConstraintId3D>>;
    private readonly _constraintBodies: Map<ConstraintId3D, [BodyId3D, BodyId3D]>;
    ...
    constructor(maxConstraints: number = 2048) {
        ...
        this._constraintIdToIndex = new Map();
        this._bodyToConstraints = new Map();
        this._constraintBodies = new Map();
...
export class PhysicsWorld3D implements Disposable {
    ...
    private _integrateVelocities(dt: number): void {
        ...
        for (const bodyId of bodyIds) {
            ...
            const vel = this._bodyManager.getLinearVelocity(bodyId);

            this._bodyManager.setLinearVelocity(bodyId, {
                x: vel.x + gravityX * gravityScale,
                y: vel.y + gravityY * gravityScale,
                z: vel.z + gravityZ * gravityScale,
            });
        }
    }
    ...
    private _integratePositions(dt: number): void {
        ...
        for (const bodyId of bodyIds) {
            ...
            const angVel = this._bodyManager.getAngularVelocity(bodyId);
            ...
            if (angSpeed > 1e-10) {
                ...
                const dqx = angVel.x * s;
                const dqy = angVel.y * s;
                const dqz = angVel.z * s;
                const dqw = c;

                const newW = dqw * rot.w - dqx * rot.x - dqy * rot.y - dqz * rot.z;
                const newX = dqw * rot.x + dqx * rot.w + dqy * rot.z - dqz * rot.y;
                const newY = dqw * rot.y - dqx * rot.z + dqy * rot.w + dqz * rot.x;
                const newZ = dqw * rot.z + dqx * rot.y - dqy * rot.x + dqz * rot.w;

                const len = Math.sqrt(newX * newX + newY * newY + newZ * newZ + newW * newW);
